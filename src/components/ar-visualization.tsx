...
export function ARVisualization({ rechargeDepth, rechargeWidth, rechargeLength }: ARViewProps) {
  ...
  useEffect(() => {
    ...
    function drawAR() {
      ...
      ctx.fillText(`${t("arRechargePit")}: ${rechargeLength}m × ${rechargeWidth}m × ${rechargeDepth}m`, cx, cy - pitL / 2 - pitD - 12);
      ...
      ctx.fillText(`${t("dimensions")}: ${rechargeLength}×${rechargeWidth}×${rechargeDepth}m`, W - 192, 44);
      ctx.fillText(`${t("volume")}: ${(rechargeLength * rechargeWidth * rechargeDepth).toFixed(1)} m³`, W - 192, 58);
  ...
  }, [arActive, rechargeDepth, rechargeWidth, rechargeLength, t]);
